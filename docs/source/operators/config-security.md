...
## Transport Encryption of Kernel Channels (CurveZMQ)

By default, the five ZeroMQ kernel sockets carry unencrypted traffic,
so any process able to reach the kernel host's ports can subscribe to `iopub` and read code and outputs.
Gateway Provisioners supports the
[transport encryption](https://jupyter-client.readthedocs.io/en/stable/security.html)
introduced in `jupyter_client` 8.9, which wraps these sockets in
[CurveZMQ](https://rfc.zeromq.org/spec/26/) encryption.
Because kernels run remotely, the keys are generated by the kernel launcher on the kernel host
and returned to the server inside the already-encrypted response payload.

The policy is configured on the kernel manager:

```python
c.KernelManager.transport_encryption = "auto"
```

The setting requires `jupyter_client` 8.9 or newer on the server;
older versions ignore it as an unrecognized option and start every kernel unencrypted.

- `disabled` (default): no keys are provisioned.
- `auto`: kernels whose kernelspec advertises support start encrypted, others start unencrypted.
  A launcher unable to honor the request (for example, an older kernel image)
  falls back to an unencrypted connection with a server-side warning.
- `required`: kernels that do not advertise support are refused,
  and the launch fails when the launcher does not return keys.
  A launcher that refuses on its own (for example, an image with an older `ipykernel`)
  exits before sending connection information, so the server reports a launch timeout
  after the provisioner's `launch_timeout` (30 seconds by default) and the reason is only in the kernel log.

The Python kernel specifications generated by the CLI tooling advertise support
and pass the policy to the launcher via the `--transport-encryption` argument.
...
[jupyter_client security documentation](https://jupyter-client.readthedocs.io/en/stable/security.html);
in addition, Python kernel environments and images need `ipykernel` 7.3 or newer.
R and Scala kernels do not currently support CurveZMQ,
so their specifications do not advertise it and `required` will refuse to start them.

```{note}
CurveZMQ protects the five kernel channels;
the communication socket used for signalling remains unencrypted.
```
